...
class testminimize_total_distance(unittest.TestCase):
    ...
    # In this test we define that each customer can be sourced by only one warehouse
        ...
        sln = minimize_total_distance(dat)
    ...
    # In this test we run the same engine using an extremely big constraint, and a no constraint setting. They should give the same output
        ...
        def test_capacitybigenoughnochanges(self):
            ...
            sln1 = minimize_total_distance(self.dat)
            ...
            self.dat = calculate_high_service_dist_par(self.dat)
            self.dat = calculate_max_dist_par(self.dat)
            sln2 = minimize_total_distance(self.dat)
    ...
    # In this test we make sure that maximum average distance constraint works
        ...
        #identify high service satisfied demand
        sln.customer_assignment['Binary Value'] = sln.customer_assignment['Distance'].apply(lambda x: 1 if float(x) > int(params['High Service Distance']) else 0)

        self.assertTrue((sln.customer_assignment['Binary Value'] * sln.customer_assignment['Customer Demand']).sum() > params['High Service Demand (If constraint active)'])
