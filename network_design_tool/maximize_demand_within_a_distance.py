...
def maximize_demand_within_a_distance(dat):
    ...
    """
    High service distance pars are calculated to understand the routes to maximize
    """
    ...
    if maximum_distance_constraint_active == "True":
        for w in warehouses:
            for c in customers:
                maximize_demand_within_a_distance += LpConstraint(e = flow_vars[w,c],
                                        sense=LpConstraintLE,
    ...
    if fixed_warehouses_constraint == "True":
        for w in warehouses:
            ...
            maximize_demand_within_a_distance += LpConstraint(e = facility_vars[w],
   ...
   # We will ensure that every output is delivered to the same output folder
    ...
    maximize_demand_within_a_distance.writeLP(lp_file_path)
