# Model_5. This model minimizes the total number of warehouses
...
def minimize_total_warehouses(dat):
    ...
    minimize_total_warehouses = LpProblem("minimize_total_warehouses", LpMinimize)
    ...
    """
    # This is the problem's objective function defined which is the total cost the model seeks to minimize
    total_warehouses = lpSum(facility_vars[w,c] for w in warehouses for c in customers)
    ...
    if capacity_constraint_active == "Every Warehouse the same Capacity" or capacity_constraint_active == "Capacity Specific for each Capacity":
        for w in warehouses:
            ...
            minimize_total_warehouses += LpConstraint(e = lpSum([customer_demands[c] * flow_vars[w, c] for c in customers]) - warehouse_capacity * lpSum(facility_vars[w,c] for c in customers),
    ...
    if avg_service_distance_constraint_active == "True":
        minimize_total_warehouses += LpConstraint(e = lpSum([distance[w,c] * customer_demands[c] * flow_vars[w,c] for w in warehouses for c in customers]),
                                        sense=LpConstraintLE,
                                        name="_Avg_Served",
    ...
    if maximum_distance_constraint_active == "True":
        for w in warehouses:
            for c in customers:
                minimize_total_warehouses += LpConstraint(e = facility_vars[w, c],
                                        sense=LpConstraintLE,
    ...
    if high_service_distance_constraint_active == "True":
        minimize_total_warehouses += LpConstraint(e = lpSum([high_service_dist_par[w,c]*customer_demands[c]*flow_vars[w,c] for w in warehouses for c in customers]),
    ...
    for c in customers:
        minimize_total_warehouses += LpConstraint(e = lpSum([flow_vars[w,c] for w in warehouses]),
    ...
    for w in warehouses:
        minimize_total_warehouses += LpConstraint(e = (lpSum([flow_vars[w,c] for c in customers]) - len(customers) * lpSum(facility_vars[w,c] for c in customers)),
                                        sense = LpConstraintLE,
                                        name = str(w)+"_Open",
                                        rhs = 0)
    ...
    for w in warehouses:
        minimize_total_warehouses += LpConstraint(e = lpSum(facility_vars[w,c] for c in customers),
                                        sense=LpConstraintLE,
                                        name=str(w)+"_Fixed",
                                        rhs = 1)
    ...
    minimize_total_warehouses.setObjective(total_warehouses)

    # The problem data is written to an .lp file
    minimize_total_warehouses.writeLP("minimize_total_warehouses.lp")

    # The problem is solved using PuLP's choice of Solver
    _solver = pulp.PULP_CBC_CMD(keepFiles=False,gapRel=0.00,timeLimit=120, msg=True)
    minimize_total_warehouses.solve(solver=_solver)
    ...
    total_distance = sum(distance[w,c]*facility_vars[w,c].varValue for c in customers for w in warehouses)
    ...
    df_cu_copy = df_cu_copy.astype({'Warehouse ID': str, 'Customer ID': str, 'Customer Demand': 'Float64', 'Distance': 'Float64', 'Warehouse Lat': 'Float64', 'Warehouse Lon': 'Float64', 'Customers Lat': 'Float64', 'Customers Lon': 'Float64', 'Transportation Cost': 'Float64', 'Rates': 'Float64'})
    ...
    """
    #Header is printed
    print("\nMinimize Total Warehouses Model")

    # The status of the solution is printed to the screen
    print ("Status:", LpStatus[minimize_total_warehouses.status])
    #file.write('\nstatus:'+ LpStatus[minimize_total_warehouses.status])
    print("Optimization Status",LpStatus[minimize_total_warehouses.status] )
    ...
    print("Objective(Total Cost): ", value(minimize_total_warehouses.objective))
    ...
    file.write("\nObjective(Total Cost): "+ str(value(minimize_total_warehouses.objective)))
