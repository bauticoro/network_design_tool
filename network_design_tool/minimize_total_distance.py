...
def minimize_total_distance(dat):
    ...
    flow_vars = LpVariable.dicts("Flow", [(w,c) for w in warehouses for c in customers], lowBound=0)
    ...
    if fixed_warehouses_constraint == "True":
        for w in warehouses:
            ...
            minimize_total_distance += LpConstraint(e = facility_vars[w],
                                sense=LpConstraintGE,
   ...
   # We will ensure that every output is delivered to the same output folder
    ...
    minimize_total_distance.writeLP(lp_file_path)
    ...
    if LpStatus[minimize_total_distance.status] == "Infeasible":
        sln = output_schema.PanDat()
        sln.status = LpStatus[minimize_total_distance.status], value(minimize_total_distance.objective)
    ...
    for (w,c) in flow_vars.keys():
        if flow_vars[(w,c)].varValue > 0:
            ...
            if dat.rates['Rate'].count() > 0:
                cust['Transportation Cost'] = flow_vars[(w,c)].varValue * rates[w,c]
                cust['Rates'] = rates[w,c]
    ...
    df_cu_copy = df_cu_copy.astype({'Warehouse ID': str, 'Customer ID': str, 'Customer Demand': 'Float64', 'Distance': 'Float64', 'Warehouse Lat': 'Float64', 'Warehouse Lon': 'Float64', 'Customers Lat': 'Float64', 'Customers Lon': 'Float64'})
    if dat.rates['Rate'].count() > 0:
        df_cu_copy = df_cu_copy.astype({'Transportation Cost': 'Float64', 'Rates': 'Float64'})
    ...
    if dat.rates['Rate'].count() > 0:
        ...
        sln.customer_assignment = df_cu_copy[['Warehouse ID', 'Customer ID', 'Customer Demand', 'Distance', 'Warehouse Lat', 'Warehouse Lon', 'Customers Lat', 'Customers Lon']]
    ...
    # We define the path for our excel file
