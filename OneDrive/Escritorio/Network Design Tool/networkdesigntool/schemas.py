...
input_schema = PanDatFactory(
    ...
    warehouses=[['Warehouse ID'], ['Name', 'City', 'State', 'Zipcode', 'Lat', 'Lon', 'Production Capacity']],
    customers=[['Customer ID'], ['Name', 'City', 'State', 'Zipcode', 'Lat', 'Lon']],
    customer_demand=[['Customer ID'], ['Demand Value']],
...
input_schema.add_parameter('Capacity Constraint Type', default_value='No Constraint', number_allowed=False,
                           strings_allowed=['No Constraint', 'Every Warehouse the same Capacity', 'Capacity Specific for each Capacity'])
...
input_schema.add_parameter('Recieve from Only One', default_value='True', number_allowed=False,
                           strings_allowed=['True', 'False'])

input_schema.add_parameter('High Service Distance-Demand Constraint', default_value='True', number_allowed=False,
                           strings_allowed=['True', 'False'])
...
input_schema.set_data_type(table=table, field='Rate', number_allowed=True, strings_allowed=(),
                           must_be_int=False, min=0, inclusive_min=True, max=float('inf'), inclusive_max=False)
...
input_schema.set_data_type(table=table, field='Binary Value', number_allowed=True, strings_allowed=(),
                           must_be_int=True, min=0, inclusive_min=True, max=1, inclusive_max=True)
...
output_schema.set_data_type(table=table, field='Warehouse Lon', number_allowed=True, strings_allowed=(),
                           must_be_int=False, min=-180, inclusive_min=True, max=180, inclusive_max=True)
output_schema.set_data_type(table=table, field='Customer Lat', number_allowed=True, strings_allowed=(),
                           must_be_int=False, min=-90, inclusive_min=True, max=90, inclusive_max=True)
output_schema.set_data_type(table=table, field='Customer Lon', number_allowed=True, strings_allowed=(),
                           must_be_int=False, min=-180, inclusive_min=True, max=180, inclusive_max=True)
