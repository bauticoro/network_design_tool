...
def minimize_total_distance(dat):
    ...
    minimize_total_distance = LpProblem ( "minimize_total_distance", LpMinimize)
    ...
    total_distance = lpSum([distance[w,c]*flow_vars[w,c] for w in warehouses for c in customers])
    ...
    if capacity_constraint_active == "Every Warehouse the same Capacity" or capacity_constraint_active == "Capacity Specific for each Capacity":
        for w in warehouses:
            ...
            minimize_total_distance += LpConstraint(e = lpSum([customer_demands[c] * flow_vars[w, c] for c in customers]) - warehouse_capacity * lpSum(facility_vars[w,c] for c in customers),
    ...
    if avg_service_distance_constraint_active == "True":
        minimize_total_distance += LpConstraint(e = lpSum([distance[w,c] * customer_demands[c] * flow_vars[w,c] for w in warehouses for c in customers]),
                                        sense=LpConstraintLE,
                                        name="_Avg_Served",
    ...
    if maximum_distance_constraint_active == "True":
        for w in warehouses:
            for c in customers:
                minimize_total_distance += LpConstraint(e = facility_vars[w, c],
                                        sense=LpConstraintLE,
    ...
    if high_service_distance_constraint_active == "True":
        minimize_total_distance += LpConstraint(e = lpSum([high_service_dist_par[w,c]*customer_demands[c]*flow_vars[w,c] for w in warehouses for c in customers]),
    ...
    for c in customers:
        minimize_total_distance += LpConstraint(e = lpSum([flow_vars[w,c] for w in warehouses]),
    ...
    if number_of_whs_constraint_active == "True":
        minimize_total_distance += LpConstraint(
    ...
    for w in warehouses:
        minimize_total_distance += LpConstraint(e = (lpSum([flow_vars[w,c] for c in customers]) - len(customers) * lpSum(facility_vars[w,c] for c in customers)),
                                        sense = LpConstraintLE,
                                        name = str(w)+"_Open",
                                        rhs = 0)
    ...
    for w in warehouses:
        minimize_total_distance += LpConstraint(e = lpSum(facility_vars[w,c] for c in customers),
                                        sense=LpConstraintLE,
                                        name=str(w)+"_Fixed",
                                        rhs = 1)
    ...
    minimize_total_distance.setObjective(total_distance)

    # The problem data is written to an .lp file
    minimize_total_distance.writeLP("minimize_total_distance.lp")

    # The problem is solved using PuLP's choice of Solver
    _solver = pulp.PULP_CBC_CMD(keepFiles=False,gapRel=0.00,timeLimit=120, msg=True)
    minimize_total_distance.solve(solver=_solver)
    ...
    print ("Status:", LpStatus[minimize_total_distance.status])
    #file.write('\nstatus:'+ LpStatus[minimize_total_distance.status])
    print("Optimization Status",LpStatus[minimize_total_distance.status] )
    ...
    print("Objective(Total Distance): ", value(minimize_total_distance.objective))
    ...
    file.write("\nObjective(Total Distance): "+ str(value(minimize_total_distance.objective)))
