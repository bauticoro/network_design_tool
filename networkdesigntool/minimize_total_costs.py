...
def minimize_total_costs(dat):
    ...
    total_cost = lpSum([rates[w,c] * flow_vars[w,c] * customer_demands[c] for w in warehouses for c in customers])
    ...
    if capacity_constraint_active == "Every Warehouse the same Capacity" or capacity_constraint_active == "Capacity Specific for each Capacity":
        for w in warehouses:
            ...
            minimize_total_costs += LpConstraint(e = lpSum([customer_demands[c] * flow_vars[w, c] for c in customers]) - warehouse_capacity * lpSum(facility_vars[w,c] for c in customers),
    ...
    if avg_service_distance_constraint_active == "True":
        minimize_total_costs += LpConstraint(e = lpSum([distance[w,c] * customer_demands[c] * flow_vars[w,c] for w in warehouses for c in customers]),
                                        sense=LpConstraintLE,
                                        name="_Avg_Served",
    ...
    if maximum_distance_constraint_active == "True":
        for w in warehouses:
            for c in customers:
                minimize_total_costs += LpConstraint(e = facility_vars[w, c],
                                        sense=LpConstraintLE,
    ...
    if high_service_distance_constraint_active == "True":
        minimize_total_costs += LpConstraint(e = lpSum([high_service_dist_par[w,c]*customer_demands[c]*flow_vars[w,c] for w in warehouses for c in customers]),
    ...
    for c in customers:
        minimize_total_costs += LpConstraint(e = lpSum([flow_vars[w,c] for w in warehouses]),
    ...
    for w in warehouses:
        minimize_total_costs += LpConstraint(e = (lpSum([flow_vars[w,c] for c in customers]) - len(customers) * lpSum(facility_vars[w,c] for c in customers)),
                                        sense = LpConstraintLE,
                                        name = str(w)+"_Open",
                                        rhs = 0)
    ...
    for w in warehouses:
        minimize_total_costs += LpConstraint(e = lpSum(facility_vars[w,c] for c in customers),
                                        sense=LpConstraintLE,
                                        name=str(w)+"_Fixed",
                                        rhs = 1)
    ...
    minimize_total_costs.writeLP("minimize_total_costs.lp")
    ...
    file.write("\nObjective(Total Cost): "+ str(value(minimize_total_costs.objective)))
